...
# ]
# ///
"""SIP Shopify support voice agent with post-session LiveKit judges.

This is the same SIP transport pattern as sip_agent.py, with one addition:
the JobContext is configured with a judge LLM and LiveKit SDK Judge objects.
After each call ends, agent-transport builds the LiveKit SessionReport, runs
...
Required for calls:
    SIP_USERNAME
    SIP_PASSWORD
    SIP_DOMAIN              # optional, defaults to phone.plivo.com
    OPENAI_API_KEY
    DEEPGRAM_API_KEY
...
from agent_transport.sip.livekit import AgentServer, EvaluationConfig, JobContext, JobProcess
...
logger = logging.getLogger("sip-agent-with-judges")
        ...
        *,
        ...
        return JudgmentResult(
            ...
            reasoning=(
                ...
                else "The call ended before the final user turn received an assistant response."
            ),
            instructions=(
    ...
    def __init__(self) -> None:
        super().__init__(
            instructions=(
                ...
                "before creating a return label. Use get_store_policy for policy "
                "questions. Do not invent order data; if a tool cannot verify it, "
                "say you need the caller to confirm the details. Do not use emojis, "
