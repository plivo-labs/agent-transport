...
background audio through Rust's RTP send loop.

Usage:
    from agent_transport.sip.pipecat import SoundfileMixer

    mixer = SoundfileMixer(transport, sound_files={"hold": "music.wav"}, default_sound="hold")
...
class SoundfileMixer(BaseAudioMixer):
    """Rust-backed soundfile mixer for SIP calls.

    Drop-in replacement for pipecat.audio.mixers.SoundfileMixer.
    Feeds background audio to Rust's RTP send loop via send_background_audio().
