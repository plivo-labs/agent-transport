"""Rust-backed audio mixers for Pipecat (shared SIP + audio_stream impl).

Drop-in replacement for pipecat.audio.mixers.SoundfileMixer that routes
background audio through Rust's send loop instead of mixing in Python.
Instead of mixing audio in Python (numpy) on the hot path, this feeds
background audio to Rust via ``send_background_audio()``; Rust mixes it with
the agent voice at 20ms intervals before encoding — zero GIL overhead on the
hot path. ``mix()`` returns the agent audio unchanged (passthrough); the
actual mixing happens in Rust at the wire level.

This module is transport-agnostic: it works against any object exposing
``send_background_audio(bytes, sample_rate, num_channels)`` (SipTransport or
AudioStreamTransport). The package-specific ``sip/pipecat/mixers.py`` and
``audio_stream/pipecat/mixers.py`` are thin re-exports of this module.

Usage:
    from agent_transport.sip.pipecat import SoundfileMixer            # SIP
    from agent_transport.audio_stream.pipecat import SoundfileMixer   # Plivo

    mixer = SoundfileMixer(
        transport,
        sound_files={"hold": "music.wav"},
        default_sound="hold",
    )
...
class SoundfileMixer(BaseAudioMixer):
    ...
    Drop-in replacement for pipecat.audio.mixers.SoundfileMixer. Feeds
    background audio to Rust's send loop via send_background_audio().
    mix() returns agent audio unchanged — Rust mixes at the wire level.
    """
    ...
    async def _feed_loop(self):
        """Feed background audio chunks to Rust at 20ms intervals."""
